...
"""Pure argparse ``type=`` validators / coercers.

Leaf module: depends only on stdlib, hcloud domain types, and the vendored
ordered_set. It must NOT import the ``config`` package or ``args`` — lower
layers (config schema, providers) import validators from here so they never
